...
import { MobileNavCreateButton } from './MobileNavCreateButton'
import { MobileCreateModal } from './MobileCreateModal'
import { MobileCreateModalProvider } from './mobileCreateModalContext'
import { useMobileBackTarget } from './useMobileBackTarget'
...
import { DataInvalidationProvider } from '../../contexts/DataInvalidationContext'
...
export function AppLayout() {
  ...
  const compactNativeTabBar = nativeShell && narrow
  const mobileBackTarget = useMobileBackTarget()
  const mobileNavBar = useMobileNavBar()
  const showMobileBack = nativeIOS && narrow && mobileBackTarget !== null
  const showNativeMobileNav = compactNativeTabBar && mobileNavBar !== null && !showMobileBack
  const [reducedMotion, setReducedMotion] = useState(false)
  const showRouteSlide = nativeIOS && narrow && !reducedMotion
        ...
        <AssistantModalRoot>
          <div className="min-h-screen flex flex-col md:flex-row bg-gray-50 print:bg-white">
            ...
            >
              <header className="fixed inset-x-0 top-0 z-10 flex h-[calc(4rem+env(safe-area-inset-top,0px))] flex-row items-center justify-between gap-3 border-b border-gray-200 bg-white px-2 pb-3 pt-[calc(env(safe-area-inset-top,0px)+0.5rem)] md:hidden print:hidden">
                {showMobileBack ? (
                  <div className="min-w-0 flex-1">
                    <MobileBackButton target={mobileBackTarget} />
                  </div>
                ) : showNativeMobileNav ? (
                  <h1 className={`${pageTitleClassName} min-w-0 flex-1`}>{mobileNavBar.title}</h1>
                ) : (
                  <h1 className={`${pageTitleClassName} min-w-0 flex-1`}>Send Invoices Online</h1>
                )}
                {narrow ? (
                  <div className="flex shrink-0 items-center gap-1">
                    {showNativeMobileNav && mobileNavBar.createTarget ? (
                      <MobileNavCreateButton target={mobileNavBar.createTarget} />
                    ) : null}
              ...
              <main
                className={[
                  'flex-1 flex flex-col min-h-0 min-w-0 md:p-4 md:px-6 print:py-6 print:px-6 print:pb-6',
                  compactNativeTabBar ? 'max-md:px-0 max-md:py-0' : 'px-2 py-4',
                  compactNativeTabBar
              ...
              >
                ...
                >
                  <AnimatedOutlet enabled={showRouteSlide} />
                </div>
                ...
                {!compactNativeTabBar ? (
                  <div className="px-3 py-2 flex items-center justify-between gap-2 border-b border-gray-100">
                    ...
                    <button
                      type="button"
                      onClick={handleSignOut}
                      className="px-2 py-1.5 rounded-lg text-xs font-medium text-gray-600 hover:bg-gray-50 shrink-0"
                    >
                      Log out
                    </button>
                  </div>
